...
async def process_video_optimized(lead_id: str, work_item: str):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # DELETE THE 500MB RAW FILE IMMEDIATELY
        if work_item.startswith("LOCAL:") and os.path.exists(raw_path):
            os.remove(raw_path)
            logger.info(f"VIDEO_ENGINE: [Cleanup] Deleted raw local file: {raw_path}")
